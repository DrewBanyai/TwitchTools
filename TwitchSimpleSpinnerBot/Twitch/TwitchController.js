...
class TwitchController {
    ...
    static async Connect(twitchChannel, twitchToken) {
        ...
        try {
            ...
            if (!twitchChat._userState) {
                console.warn("Invalid user state returned when connecting to channel!")
                return false
            }
    ...
    static handleTwitchMessage(message) {
        ...
        if (twitchMessageCallbacks.hasOwnProperty(message.event)) {
            if (twitchMessageCallbacks[message.event](message))
            return
        }

        //  Provide a default response (usually a console log) for different Twitch chat events
